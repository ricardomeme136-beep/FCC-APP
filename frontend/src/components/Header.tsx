    ...
    <View style={[styles.wrap, { paddingTop: insets.top + spacing.sm, backgroundColor: bg }]}>
      <View style={styles.row}>
        <View style={styles.left}>
          {back ? (
            <Pressable testID="header-back" onPress={() => router.back()} style={styles.backBtn} hitSlop={10}>
              <Ionicons name="chevron-back" size={24} color={fg} />
            </Pressable>
          ) : null}
          <View style={{ flexShrink: 1 }}>
            {subtitle ? <Txt variant="label" color={dark ? "#A1A1AA" : colors.muted}>{subtitle}</Txt> : null}
